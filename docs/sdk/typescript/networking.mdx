...
interface Rule {
  ...
  readonly methods?: readonly HttpMethod[]; // empty / omitted = any
  readonly paths?: readonly string[];       // empty / omitted = any
...
Add an HTTP method (RFC 9110 plus `PATCH` / `QUERY`). Empty set means any method. Evaluated for plaintext HTTP/1 and for HTTP/1 and HTTP/2 decrypted by TLS interception.

#### <span className="msb-recv">rule.</span><span className="msb-hn">methods()</span>

```typescript
methods(methods: HttpMethod[]): this
```
...
Add an HTTP origin-form path. Empty set means any path. Matching is exact after query and fragment are stripped.

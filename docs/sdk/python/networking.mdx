...
`tuple[`[`HttpMethod`](#httpmethod)`, ...]` · Default: `()`

HTTP method set; empty means any method. Evaluated for plaintext HTTP/1 and for HTTP/1 and HTTP/2 decrypted by TLS interception. Encrypted HTTPS without interception cannot match these filters. RFC 9110 methods plus `PATCH` and `QUERY`

#### <span className="msb-recv">rule.</span><span className="msb-hn">paths</span>

`tuple[str, ...]` · Default: `()`

HTTP origin-form path set; empty means any path. Exact match after query and fragment are stripped
...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>methods</code><span className="msb-type">Iterable[HttpMethod]</span></div>
    <div className="msb-param-desc">HTTP method set. Empty means any method.</div>
  </div>
  <div className="msb-param">
    <div className="msb-param-key"><code>paths</code><span className="msb-type">Iterable[str]</span></div>
    <div className="msb-param-desc">HTTP origin-form path set. Empty means any path.</div>
  </div>
</div>

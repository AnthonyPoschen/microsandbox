...
#### <span className="msb-recv">rule.</span><span className="msb-hn">method()</span>

```rust
fn method(&mut self, method: HttpMethod) -> &mut Self
```

Add an HTTP method to the method set. Empty set means any method. Matching is ASCII case-insensitive.

#### <span className="msb-recv">rule.</span><span className="msb-hn">methods()</span>

```rust
fn methods<I: IntoIterator<Item = HttpMethod>>(&mut self, methods: I) -> &mut Self
```

Add several HTTP methods. Equivalent to calling [`method()`](#rule-method) once per element.

#### <span className="msb-recv">rule.</span><span className="msb-hn">path()</span>

```rust
fn path(&mut self, path: impl Into<String>) -> &mut Self
```

Add an HTTP origin-form path. Empty set means any path. Matching is exact after query and fragment are stripped from the request-target.

#### <span className="msb-recv">rule.</span><span className="msb-hn">paths()</span>

```rust
fn paths<I, S>(&mut self, paths: I) -> &mut Self
where
    I: IntoIterator<Item = S>,
    S: Into<String>,
```

Add several HTTP paths. Equivalent to calling [`path()`](#rule-path) once per element.
...
`Vec<`[`HttpMethod`](#httpmethod)`>`

Set semantics; empty = any HTTP method. Evaluated for plaintext HTTP/1 and for HTTP/1 and HTTP/2 decrypted by TLS interception. Encrypted HTTPS without interception cannot match these filters. RFC 9110 methods plus `PATCH` and `QUERY`. Omitted filters keep SYN-time L4 matching unchanged.

#### <span className="msb-recv">Rule::</span><span className="msb-hn">paths</span>

`Vec<String>`

Set semantics; empty = any path. Exact origin-form path match after query and fragment are stripped.
...
<p className="msb-backref">Held by <a href="#rule">Rule.methods</a> · set by <a href="#rule-method">RuleBuilder method setters</a></p>
...
Empty `methods` means any HTTP method. Filters apply to plaintext HTTP/1 and to HTTP/1 and HTTP/2 decrypted by TLS interception.

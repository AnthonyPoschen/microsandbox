...
Set semantics; empty = any HTTP method. Evaluated only for plaintext HTTP/1 request lines. RFC 9110 methods plus `PATCH` and `QUERY`. Omitted filters keep SYN-time L4 matching unchanged.
...
Empty `methods` means any HTTP method. Filters apply only to plaintext HTTP/1 request lines.

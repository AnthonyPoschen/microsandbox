...
Running sandboxes expose structured enforcement events through [`Sandbox.NetworkDecisions`](#m-networkdecisions) and [`Sandbox.NetworkDecisionStream`](#m-networkdecisionstream). Events cover DNS, TCP, UDP, TLS/SNI, and HTTP policy decisions. Bodies, credentials, cookies, and TLS private material are never included.
...
| Methods | `[]`[`PolicyHttpMethod`](#policyhttpmethod) | HTTP method set. Empty means any. Evaluated for plaintext HTTP/1 and for HTTP/1 and HTTP/2 decrypted by TLS interception. RFC 9110 methods plus `PATCH` and `QUERY` |
| Paths | `[]string` | HTTP origin-form path set. Empty means any. Exact match after query and fragment are stripped |
...
<p className="msb-backref">Used by <a href="#policyrule">PolicyRule.Methods</a></p>

```go
type PolicyHttpMethod string
```

HTTP method filter. Empty `Methods` means any method. Evaluated for plaintext HTTP/1 and for HTTP/1 and HTTP/2 decrypted by TLS interception. RFC 9110 methods plus `PATCH` and `QUERY`.

| Constant | Value |
|----------|-------|
| `PolicyHttpMethodGET` | `"GET"` |
| `PolicyHttpMethodHEAD` | `"HEAD"` |
| `PolicyHttpMethodPOST` | `"POST"` |
| `PolicyHttpMethodPUT` | `"PUT"` |
| `PolicyHttpMethodDELETE` | `"DELETE"` |
| `PolicyHttpMethodCONNECT` | `"CONNECT"` |
| `PolicyHttpMethodOPTIONS` | `"OPTIONS"` |
| `PolicyHttpMethodTRACE` | `"TRACE"` |
| `PolicyHttpMethodPATCH` | `"PATCH"` |
| `PolicyHttpMethodQUERY` | `"QUERY"` |
...
Every DNS, TCP, UDP, TLS/SNI, and HTTP enforcement decision is recorded in a bounded per-sandbox ring buffer. Replay it with `Sandbox.NetworkDecisions`, or follow live events with `Sandbox.NetworkDecisionStream`. `Recv` returns `nil, nil` at end-of-stream (snapshot drained or sandbox stopped). Events never include request bodies, credentials, cookies, or TLS private material.

```go
snap, err := sb.NetworkDecisions(ctx, m.NetworkDecisionOptions{})
stream, err := sb.NetworkDecisionStream(ctx, m.NetworkDecisionOptions{Follow: true})
defer stream.Close()
for {
    ...
    fmt.Println(ev.Phase, ev.Action, ev.Reason, ev.Sequence)
}
```

Capacity is configured in host runtime configuration as `runtime.network_decision_buffer_capacity` (default 4096). Wrapped events increment `dropped_count` rather than disappearing silently.

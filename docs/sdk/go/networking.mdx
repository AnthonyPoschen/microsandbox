...
| Methods | `[]`[`PolicyHttpMethod`](#policyhttpmethod) | HTTP method set. Empty means any. RFC 9110 methods plus `PATCH` and `QUERY` |
...
HTTP/1 method filter. Empty `Methods` means any method. RFC 9110 methods plus `PATCH` and `QUERY`.
